{
  # Already has its own dotfiles auto-sync from Forgejo with Pushover notifications and approval-gated reboot
  # toggle at services.dotfiles-sync.enable, so it isn't wrapped in a dotfiles.<name>.enable option like the
  # other aspects in this directory.
  flake.modules.nixos.dotfiles-sync = {
    config,
    lib,
    pkgs,
    ...
