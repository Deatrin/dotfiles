{
  flake.modules.nixos.reboot-required = {
    config,
    lib,
    ...
  }: let
    cfg = config.dotfiles.reboot-required;
  in {
    options.dotfiles.reboot-required.enable = lib.mkEnableOption "hourly reboot-required check timer";
