...
{
  flake.modules.nixos.traefik-forward-auth = {
    config,
    pkgs,
    lib,
    ...
  }: let
    cfg = config.dotfiles.traefik-forward-auth;
    inherit (config.virtualisation.quadlet) networks;
  in {
    options.dotfiles.traefik-forward-auth.enable = lib.mkEnableOption "Traefik Forward Auth OIDC middleware";
