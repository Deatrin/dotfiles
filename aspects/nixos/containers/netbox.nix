...
#   netbox.jennex.dev → netbox:8080
{
  flake.modules.nixos.netbox = {
    ...
    cfg = config.dotfiles.netbox;
    inherit (config.virtualisation.quadlet) networks;
    domain = "netbox.jennex.dev";
    extraPy = pkgs.writeText "netbox-extra.py" ''
      # Extra NetBox configuration — secrets read from /run/opnix/ at Django startup
    ...
    '';
  in {
    options.dotfiles.netbox.enable = lib.mkEnableOption "NetBox network source of truth";
