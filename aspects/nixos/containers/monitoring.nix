...
#
# Dashboard JSON files live alongside the old module at
# hosts/common/optional/containers/monitoring/dashboards/ (not moved — only .nix
# files are auto-discovered by import-tree, so the JSON assets stay in place).
{
  flake.modules.nixos.monitoring = {
    ...
    dashboardsDir = ../../../hosts/common/optional/containers/monitoring/dashboards;
    ...
    '';
  in {
    options.dotfiles.monitoring.enable = lib.mkEnableOption "Monitoring stack (Prometheus, Grafana, Loki, Alloy, UnPoller)";
