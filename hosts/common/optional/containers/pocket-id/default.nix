...
in {
  ...
  virtualisation.quadlet.containers.pocket-id = {
    ...
    containerConfig = {
      ...
      environments = {
        APP_URL = "https://pocket-id.deatrin.dev";
        TRUST_PROXY = "true";
        PUID = "1000";
        PGID = "1000";
      };
      ...
      labels = [
        ...
        "homepage.href=https://pocket-id.deatrin.dev"
        "homepage.description=OIDC Identity Provider"
        "traefik.enable=true"
        "traefik.http.routers.pocket-id.rule=Host(`pocket-id.deatrin.dev`)"
        "traefik.http.routers.pocket-id-secure.entrypoints=https"
        "traefik.http.routers.pocket-id-secure.rule=Host(`pocket-id.deatrin.dev`)"
